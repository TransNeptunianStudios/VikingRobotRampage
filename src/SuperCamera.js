SuperCamera = function (player, game) {
	...
	this.camera.deadzone = new Phaser.Rectangle(50, 0, game.width / 2, 0);

	// if the player is on the left side...
	this.isScrollingLeft = function () {
		var threshold = this.camera.x + this.camera.deadzone.x;

		if (this.player.isMoving && this.camera.x > 0 && Math.floor(this.player.x) <= threshold)
			return true;
	...
	this.isScrollingRight = function () {
		...
		if (this.player.isMoving && (this.camera.x + this.camera.width) < this.worldWidth && this.player.x >= threshold)
			return true;
